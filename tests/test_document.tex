\documentclass{article}

\usepackage{gurps}
\usepackage{blindtext}



\begin{document}

% Generating a table of contents with unstarred \charactertitle and
% \charactersection is currently broken for unknown reasons
\tableofcontents

\section{Dice notation demo}
\label{sec:dice-notation-demo}

\gurps is an excellent RPG system which can be typeset with \LaTeX{}! It uses 6
sided dice which are notated as \dice{$n$}[$m$], e.g. \dice{3}, \dice{4}[0], \dice{2}[-1], \dice{7}[+11].

\section{A character appears}
\label{sec:character-appears}

\begin{character}[foo bar]
  \ST{11}
  \IQ{12}
  \DX{8}
  \Per{15}
  \advantage{Zoology}
  \advantage{\LaTeX{} skills}[15]
  \disadvantage{Fear of lions}
  \disadvantage{Bugs}
  \disadvantage{Apathy}
  \levelledadvantage{\TeX{} wizardry}{3}
  \skill{Foo-barring}{12}
  \skill{\TeX pert!}[IQ/Wildcard]{16}
  \skill{Computer Operations (\LaTeX{})}[\TeX pert!/Hard]{16}
  \spell{\texttt{\char`\\ unexpanded}}{16}

  \meleeattack{
    name={Punch keyboard},
    level=18,
    damage={5d cr},
    reach={C,1},
    notes={Believe it!}
  }
  \meleeattack{
    name={Slam keyboard},
    level=16,
    damage={10d cr},
    reach={C,1},
    notes={Believe it \emph{more!}}
  }
  \rangedattack{
    name={Throw keyboard},
    level=21,
    damage={1d cr},
    range={100/1000},
    notes={ARRRGGGG!}
  }
\end{character}

\subsection{Then print \emph{another} character }
\label{sec:then-print-emph}

This character purposefully has no skills, advantages or disadvantages to check
that they are not printed if needed. Also, this tests defining the points as an
integer.

This test also tests the \texttt{:nolatex:} tag to make sure that
traits/skills/etc. with \texttt{:nolatex:} in the name are not added to the character.

\begin{character}
  \spell{Foo bar?}[59]{40}
  \advantage{This advantage should not be added to the character! :nolatex:}
\end{character}

\subsection{Then the character is printed again \ldots{}}
\label{sec:then-char-prepr}

\GCPrintCharacter[foo bar]

\subsection{Now we print the character again, but only part of it}
\label{sec:aasdfasdf}

\makeatletter
\SetCharacterKey{foo bar}
\luadirect{traitlistmaker(create_list_predicate(\luastring{advantages,disadvantages}))}
\ResetCharacterKey
\makeatother


\section{Various disclaimers}
\label{sec:various-disclaimers}

\subsection{Disclaimer for fan sites}
\label{sec:disclaimer-fan-sites}

\SJGamesOnlinePolicyDisclaimer

\subsection{Notice for fan sites}
\label{sec:disclaimer-fan-sites}

\SJGamesOnlinePolicyNotice

\subsection{Required text for game aids}
\label{sec:disclaimer-fan-sites}

\SJGamesOnlinePolicyGameAid{Nathanael Farley}

\end{document}
